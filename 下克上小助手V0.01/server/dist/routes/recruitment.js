...
const recruitment_1 = require("../services/recruitment");
...
router.get('/info', middleware_1.requireAuth, (req, res) => {
    ...
    const recruitInfo = (0, recruitment_1.getRecruitInfo)(factionId);
    if (!recruitInfo) {
        res.status(404).json({
            success: false,
            error: '无法获取招募信息',
 ...
 * 执行士兵招募
 */
router.post('/recruit', middleware_1.requireAuth, middleware_1.checkGameLock, (req, res) => {
    if (!req.sessionId) {
        res.status(401).json({
            success: false,
            error: '未认证',
    ...
    // 执行招募
    const result = (0, recruitment_1.recruitSoldiers)({
        factionId,
        count: Number(count),
    });
    if (!result.success) {
