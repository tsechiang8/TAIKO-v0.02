...
exports.validateLegionName = validateLegionName;
exports.checkCommanderConflict = checkCommanderConflict;
exports.createLegion = createLegion;
exports.disbandLegion = disbandLegion;
exports.updateLegionSoldiers = updateLegionSoldiers;
exports.updateLegionEquipment = updateLegionEquipment;
exports.getAvailableCommanders = getAvailableCommanders;
exports.getLegionById = getLegionById;
exports.getFactionLegions = getFactionLegions;
 ...
 * @returns 验证结果
 */
function validateLegionName(name) {
    if (!name || name.trim() === '') {
 ...
 */
function checkCommanderConflict(samuraiId, excludeLegionId) {
    const legions = (0, storage_1.getLegions)();
    return legions.find(l => l.commanderId === samuraiId && l.id !== excludeLegionId) || null;
}
/**
 * 创建军团
 ...
 * @returns 创建结果
 */
function createLegion(request, forceReassign = false) {
    const { factionId, name, commanderId, soldierCount, rifles, horses, cannons, locationId, } = request;
    ...
    const samuraiIndex = samurais.findIndex(s => s.id === commanderId && s.factionId === factionId);
    if (samuraiIndex === -1) {
    ...
    const location = territories.find(t => t.id === locationId && t.factionId === factionId);
    if (!location) {
 ...
 * @returns 解散结果
 */
function disbandLegion(legionId, factionId) {
    // 1. 查找军团
    const legions = (0, storage_1.getLegions)();
 ...
 * @returns 编辑结果
 */
function updateLegionSoldiers(request) {
 ...
 * @returns 编辑结果
 */
function updateLegionEquipment(request) {
 ...
 * @returns 可用将领列表
 */
function getAvailableCommanders(factionId) {
    const samurais = (0, storage_1.getSamurais)();
 ...
 * @returns 军团详情或null
 */
function getLegionById(legionId) {
    const legions = (0, storage_1.getLegions)();
 ...
 * @returns 军团列表
 */
function getFactionLegions(factionId) {
    const legions = (0, storage_1.getLegions)();
